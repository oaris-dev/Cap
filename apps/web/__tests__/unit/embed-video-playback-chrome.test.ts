...
// The fork renders a persistent bar beneath the player carrying the video
// title and the oaris logo, so the title never leaves the DOM entirely. Count
// occurrences instead: the overlay copy disappears, the bar's copy stays.
const titleOccurrences = (container: HTMLElement) =>
	Array.from(container.querySelectorAll("*")).filter(
		(el) => el.children.length === 0 && el.textContent === "Test video",
	).length;

const expectChromeVisible = (container: HTMLElement) => {
	expect(titleOccurrences(container)).toBeGreaterThan(1);
	expect(
...
const expectChromeHidden = (container: HTMLElement) => {
	expect(titleOccurrences(container)).toBe(1);
